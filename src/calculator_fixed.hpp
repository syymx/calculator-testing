...
private:
  ...
  checked_sub(T x, T y) const
  {
    // 缺陷：零减最小整数时未检测溢出。原因：原条件 x > 0 遗漏了 x == 0。
    if (x >= 0 && y < 0) {
      if (x > maxValue() + y)
        throw_integer_overflow_error();
    }
  ...
  {
    // 缺陷：非法移位量未抛出异常。原因：直接执行移位，未检查负数及达到位宽的移位量。
    if (op.op == OPERATOR_BITWISE_SHL || op.op == OPERATOR_BITWISE_SHR)
    {
      if ((is_signed() && v2 < 0) || v2 >= numBits())
        throw calculator::error("Error: invalid shift count!");
    }
  ...
  // 缺陷：最小负整数字面量解析失败。原因：先累积正数会超出上界，需直接累积负值。
  T parseDecimal(bool negative = false)
  {
    T value = 0;
    ...
    for (T d; (d = getInteger()) <= 9; index_++)
    {
      ...
      value = negative ? checked_sub(value, d) : checked_add(value, d);
    }
  ...
  T parseValue()
  {
    ...
    switch (getCharacter())
    {
      ...
      case '-': index_++;
                ...
                // 缺陷：最小负整数字面量解析失败。原因：原流程先解析正数再取负，取负前已溢出。
                eatSpaces();
                if (getCharacter() >= '0' && getCharacter() <= '9' &&
                    !(getCharacter() == '0' && isHex()))
                {
                  val = parseDecimal(true);
                  break;
                }
